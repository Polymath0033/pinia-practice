    ...
    <AppModalOverlay :active="active" @close="active = false">
      <div v-if="!cartStore.isEmpty">
        <ul class="items-in-cart">
          <CartItem v-for="(item, name) in cartStore.grouped" :key="name" :product="item[0]"
            :count="cartStore.groupCount(name)" @updateCount="" @clear="cartStore.removeItem(name)" />
        </ul>
        <div class="flex justify-end text-2xl mb-5">
          Total: <strong>{{ cartStore.total }} ..{{ cartStore.tot }} </strong>
        </div>
